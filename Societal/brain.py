...
def feedforward(DP,x, H, W, B): #Does a complete feedforward iteration through the network
    z = []
    A = [x]
    for i in range(len(H)-1):
        ...
        z[-1] = np.reshape( H[i], DP  )
        if i == len(H)-2: #Final layer
            A.append( z[i] )
            #A.append( softmax(z[i]) )
        else:
            A.append( np.tanh(z[i]) )
    return z,A #Only interested in the output
...
def breed(male, female, male_bias, female_bias, NR_CH, MUTATION_COEFF, H):
    ...
    for k in range(NR_CH): #Going trough all characters...
        ...
        for i in range( len(H)-1): #creates 3 weight matrices
            ...
            mutation_B = np.reshape( mutation_B, ( H[i+1],1 ))
            for row in range(H[i+1]):
                b[row,0] = np.random.choice(( male_bias[i][row,0], female_bias[i][row,0] ))
